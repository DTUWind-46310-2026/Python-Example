...
class AerodynamicsBase(ABC):
    ...
    def __init__(
        self, rho: float, CP_max: float, tsr_opt: float, omega_rated: float, P_rated: float, pitch_opt: float
    ) -> None:
        self.rho = rho
        self.CP_max = CP_max
        self.tsr_opt = tsr_opt
        self.omega_rated = omega_rated
        self.P_rated = P_rated
        self.pitch_opt = np.deg2rad(pitch_opt)
...
class Aerodynamics(AerodynamicsBase):
    ...
    def __init__(
        ...
        """
        AerodynamicsBase.__init__(
            self, rho=rho, CP_max=CP_max, tsr_opt=tsr_opt, omega_rated=omega_rated, P_rated=P_rated, pitch_opt=pitch_opt
        )
...
class NoAerodynamics(AerodynamicsBase):
    ...
    def simulation_init(self, simulation: Simulation):
        self._r = simulation.structure.r

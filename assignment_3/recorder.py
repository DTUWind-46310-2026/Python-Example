...
def flexible_DOFs_recorder(n_flexible_blades: int):
    def displacements(simulation: Simulation):
        return simulation.structure.q

    return Recorder(
        displacements,
        "flexible_displacements",
        (
            "x_t",
            "phi_shaft",
        )
        + tuple(f"q_b{b}_m{i}" for b in range(n_flexible_blades) for i in range(3)),
    )


def tower_position_recorder():
    def x_t(simulation: Simulation):
        return simulation.structure.q[0]

    return Recorder(x_t, "tower_position", "x_t")


def tower_velocity_recorder():
    def x_t_dot(simulation: Simulation):
        return simulation.structure.q_dot[0]

    return Recorder(x_t_dot, "tower_velocity", "x_t_dot")


def modal_amplitude_recorder(blade_idx: int, mode_idx: int):
    idx = 2 + 3 * blade_idx + mode_idx

    def q_k(simulation: Simulation):
        return simulation.structure.q[idx]

    return Recorder(q_k, f"q_blade_{blade_idx}_mode_{mode_idx}", "q")


def modal_velocity_recorder(blade_idx: int, mode_idx: int):
    idx = 2 + 3 * blade_idx + mode_idx

    def q_dot_k(simulation: Simulation):
        return simulation.structure.q_dot[idx]

    return Recorder(q_dot_k, f"q_dot_blade_{blade_idx}_mode_{mode_idx}", "q_dot")


def tip_deflection_recorder(blade_idx: int):
    def tip_deflections(simulation: Simulation):
        q = simulation.structure.q[2 + 3 * blade_idx : 2 + 3 * (blade_idx + 1)]
        edgewise = sum([q[i] * simulation.structure._phi_y_org[i, -1] for i in range(3)])
        flapwise = sum([q[i] * simulation.structure._phi_z_org[i, -1] for i in range(3)])
        return edgewise, flapwise

    return Recorder(tip_deflections, f"tip_deflections_{blade_idx}", ("edgewise", "flapwise"))


def root_bending_moment_recorder(blade_idx: int, r_root: float = 2.8):
    def m_root(simulation: Simulation):
        structure = simulation.structure
        r = structure.r
        mask = r >= r_root
        py = simulation.aerodynamics.py[blade_idx][mask]
        pz = simulation.aerodynamics.pz[blade_idx][mask]

        # Gravitational distributed load in the blade (y, z) frame
        g_y, g_z = structure._gravity_in_blade_frame(blade_idx)
        m = structure._blade_mass[mask]
        py = py + m * g_y
        pz = pz + m * g_z

        arm = r[mask] - r_root
        m_edge = np.trapezoid(py * arm, r[mask])
        m_flap = np.trapezoid(pz * arm, r[mask])
        return m_edge, m_flap

    return Recorder(m_root, f"root_bending_blade_{blade_idx}", ("edgewise", "flapwise"))

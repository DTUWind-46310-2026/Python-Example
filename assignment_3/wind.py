...
class WindWithTower(WindBase):
    ...
    def __init__(self, surrounding_wind: WindBase) -> None:
        ...
        """
        if isinstance(surrounding_wind, TurbulentWind):
            raise ValueError("'WindWithTower' doesn't support turbulent surrounding wind.")
    ...
    @timer
    def step(self, simulation: Simulation):
        self.surrounding_wind.step(simulation)
...
class WindSteps(WindBase):
    ...
    def step(self, simulation):
        self.base_wind.step(simulation)
        if np.any(simulation.time >= self._step_times):
            if (i := (np.argwhere(simulation.time >= self._step_times)[-1])) > self._i_current_wind:
                self._i_current_wind = i
                self._current_multiplier = self._step_multipliers[i]

    def __call__(self, xyz) -> np.ndarray:
        return self.base_wind(xyz) * self._current_multiplier

    @property
    def hub_mean(self) -> float:
        return self.base_wind.hub_mean * self._current_multiplier
...
try:
    ...
    class TurbulentWind(WindBase):
        ...
        def step(self, simulation: Simulation) -> None:
            ...
            self.mean_wind.step(simulation)

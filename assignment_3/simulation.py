...
if __name__ == "__main__":
    ...
    from structure import RigidStructure, ElasticStructure
    from wind import ConstantWind, ShearWind, TurbulentWind, WindSteps, WindWithTower

    wind = ConstantWind(7)
    # steps = [(0, 8), (100, 15)]
    # wind = WindSteps(wind, *steps)
    # wind = ShearWind(119, 11.4, 0.2)
    # wind = TurbulentWind.generate((512, 16, 16), (5, 15, 15), 0.1, wind, hub_mean=11.4)
    # wind = WindWithTower(wind)
    sim = Simulation(
        # structure=RigidStructure(7.8052 * 4 / 89.17),
        structure=ElasticStructure(0.5, n_flexible_blades=1),
        ...
        recorders=[
            ...
            aero_torque_recorder(),
            thrust_recorder(),
            tip_deflection_recorder(0),
            # generator_power_recorder(),
            # generator_torque_recorder(),
            # rotation_speed_recorder(),
            # pitch_recorder(),
            flexible_DOFs_recorder(1),
            # setpoint_pitch_recorder(),
            # tsr_recorder(),
            # controller_mode(),
            # rotation_speed_recorder(),
            # wind_1_recorder("wind_1", (60, 0, -5))
        ],
        verbose=False,
    )
    sim.run(0.05, 300, "_sim_flexible_constwind", overwrite=True)
